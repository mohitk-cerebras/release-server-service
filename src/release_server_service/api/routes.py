...
async def create_replica(request: CreateReplicaRequest):
    ...
    try:
        # ReplicaManagerV2.create_replica returns ReplicaInfo directly
        replica_info = await manager.create_replica(request)
    ...
    # Always return 201 with replica info
    # Worker process handles lifecycle independently
    message = f"Replica {replica_info.replica_id} created (worker process spawned)"
    ...
    return CreateReplicaResponse(
        ...
        request_id=replica_info.request_id,
    )
...
async def list_replicas(
    ...
    manager = get_manager()
    # ReplicaManagerV2.list_replicas returns List[ReplicaInfo] directly
...
async def get_replica_status(replica_id: str):
    ...
    manager = get_manager()
    # ReplicaManagerV2.get_replica returns ReplicaInfo directly
    replica_info = await manager.get_replica(replica_id)
    if not replica_info:
...
async def stop_replica(
    ...
    manager = get_manager()
    # ReplicaManagerV2.stop_replica returns ReplicaInfo directly
    replica_info = await manager.stop_replica(replica_id, force=request.force)
    if not replica_info:
        raise HTTPException(status_code=404, detail=f"Replica {replica_id} not found")
...
async def health_check_replica(
    ...
    if is_healthy is None:
        raise HTTPException(
            status_code=400,
            detail=f"Replica {replica_id} not found or has no health info",
        )
...
async def service_health():
    ...
    manager = get_manager()
    # manager.replicas returns dict of states
    all_states = manager.replicas
    total = len(all_states)
    ready = sum(
        1 for state in all_states.values() if state.get("status") == "ready"
    )
    return {
        "status": "ok",
